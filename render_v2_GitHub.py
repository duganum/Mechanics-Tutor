...
def render_lecture_visual(topic, params=None):
    """
    Renders dynamic beam, shear, and moment diagrams.
    Titles are context-aware to hide stress data when not relevant.
    """
    ...
    if any(kw in topic for kw in ["Shearing Forces", "Beams", "Bending"]):
        ...
        sigma_b = params.get('sigma_b') # Will be None if not provided by main.py
        ...
        # SHORTENED ARROW: xytext=0.1 ensures the red arrow is short
...
def render_problem_diagram(prob):
    """Fallback for FE Exam review problems."""
    fig, ax = plt.subplots(figsize=(3, 3))
